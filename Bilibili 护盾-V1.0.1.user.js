...
// @description  全能净化终极形态。修复CSS语法错误，强化API拦截与DOM清洗逻辑。支持图形化控制面板，自动拦截广告、营销号、综艺课堂及各类垃圾内容。
...
(function() {
    ...
    const DEFAULT_CONFIG = {
        minLevel: 2,
        debug: true,
        cleanHomepageDOM: true,   // 首页DOM清洗
        autoBlockMCN: true,       // 营销号拦截
        autoBlockMarketing: true, // 卖课拦截
        autoBlockAds: true,       // 广告拦截
        autoBlockToxic: true,     // 引战拦截
        blockZone: true,          // 分区拦截
        blockInitial: true        // 初始号拦截
    };
    ...
    function saveSetting(key, value) {
        ...
        log(`[Config] ${key} -> ${value}`, "#00a1d6");
        updateStatusText("设置已保存 (部分需刷新)", "lime");
    }
    ...
    // 初始号正则 (bili_ + 纯数字)
    const INITIAL_ACCOUNT_REGEX = /^bili_\d+$/;
    ...
    function shouldRemoveItem(item) {
        ...
        // 2. 初始号检查
        if (CONFIG.blockInitial && INITIAL_ACCOUNT_REGEX.test(ownerName)) {
             log(`[Initial] ${ownerName}`, "#ffa500");
             return { remove: true, block: false };
        }

        // 3. API分区/身份检查 (零容忍，无视白名单)
        if (CONFIG.blockZone) {
            if (BLOCK_IDENTITY.some(id => tname.includes(id) || gotoType.includes(id) || uri.includes(id))) {
                log(`[Zone] ${title} [${tname}/${gotoType}]`, "#ff00ff");
                return { remove: true, block: false };
            }
        }
        ...
        if (CONFIG.autoBlockToxic) {
            ...
            if (forceKillHit) {
                log(`[Toxic] ${title} [${forceKillHit}]`, "#ff00ff");
    ...
    function blastFloors() {
        ...
        cards.forEach(card => {
            ...
            if (link && (link.href.includes('/variety/') || link.href.includes('/cheese/'))) {
    ...
    console.log("%c Bilibili Shield V39 Active ", "background: #00a1d6; color: #fff; padding: 2px 5px; border-radius: 3px;");
})();
